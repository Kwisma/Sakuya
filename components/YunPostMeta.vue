...
import { useAppStore, useSiteConfig, useLayout } from 'valaxy'
...
<template>
  ...
  <div v-if="frontmatter" class="post-meta gap-4" flex="~ center" text="sm" :class="{
    'flex-col gap-2!': app.isMobile || frontmatter.updated,
  }">
    <YunPostDateMeta :frontmatter="frontmatter" />

    <div class="inline-flex-center gap-4">
      <div v-if="siteConfig.statistics.enable" class="inline-flex-center post-counter gap-4">
        <span v-if="frontmatter.wordCount" class="word-count inline-flex-center gap-1" :title="t('statistics.word')">
          <div class="inline-block" i-ri-file-word-line />
          <span class="op-80">{{ frontmatter.wordCount }}</span>
        </span>

        <span v-if="frontmatter.readingTime" class="reading-time inline-flex-center gap-1"
          :title="t('statistics.time')">
          <div i-ri-timer-line />
          <time class="op-80">{{ frontmatter.readingTime }}m</time>
        </span>
        <span v-if="isPost" id="busuanzi_container_page_pv">
          {{ t('post.meta.text') }}: <span id="busuanzi_value_page_pv" /> {{ t('post.meta.ci') }}
        </span>
      </div>
      <YunWalineMeta />
    </div>
